...
class BpmClient {
  ...
  private async getToken(): Promise<string> {
    ...
    const json = await resp.json() as { code?: number; message?: string; data?: { token: string; expire: number } };
  ...
  async createProcess(params: {
    modelCode: string; startUserId: string; businessKey?: string;
    processInstanceName?: string; formData: Record<string, unknown>; variables?: Record<string, unknown>;
  }) {
    return this.post('/runtime/openapi/v2/proc-insts/create', params);
  }

  async getProcess(businessKey: string) {
    return this.get('/runtime/openapi/v2/proc-insts/get', { businessKey });
  }

  async listProcessIds(params: {
    modelCode: string; pageNum: number; pageSize: number;
    startTimeBegin: string; startTimeEnd: string;
  }) {
    return this.post('/runtime/openapi/v2/proc-insts/list', params);
  }

  async terminateProcess(params: { businessKey: string; operator: string; comment?: string }) {
    return this.post('/runtime/openapi/v2/proc-insts/terminate', params);
  }

  async recallProcess(params: { businessKey: string; operator: string; comment?: string }) {
    const qs = new URLSearchParams();
    qs.set('businessKey', params.businessKey);
    qs.set('operator', params.operator);
    if (params.comment) qs.set('comment', params.comment);
    return this.post(`/runtime/openapi/v2/proc-insts/recall?${qs.toString()}`, {});
  }

  async listProcessHistory(params: Record<string, string | undefined>) {
    return this.get('/runtime/openapi/v2/proc-insts/history', params);
  }

  // --- Task APIs ---

  async getTaskHistory(businessKey: string, needPredict?: boolean) {
  ...
  async getReturnActivities(taskId: string) {
    return this.get(`/runtime/openapi/v2/task/${taskId}/returns/activities`, {});
  }

  async returnTask(params: {
    taskId: string; targetActivityId: string; operator: string; comment?: string;
  }) {
  ...
  async getTaskDetailLink(businessKey: string, assignee?: string) {
    return this.get('/runtime/openapi/v2/task/detail/link', { businessKey, assignee });
  }

  async receiveTask(params: { businessKey: string; taskDefKey: string; variables?: Record<string, unknown> }) {
    return this.post('/runtime/openapi/v2/task/receive', params);
  }

  async claimTask(params: { taskId: string; operator: string }) {
    return this.post('/runtime/openapi/v2/task/claim', params);
  }

  // --- Form APIs ---

  async getFormDefinition(formDefinitionId: string, taskDefinitionKey?: string) {
  ...
  async getFormInstance(businessKey: string, taskDefinitionKey?: string) {
...
async function main() {
  ...
  server.tool(
    'create_process',
    'Create a new BPM approval process instance.',
    {
  ...
  server.tool(
    'get_process',
    'Get details of a single process instance by businessKey.',
    { business_key: z.string().describe('Business key of the process') },
    async ({ business_key }) => {
      try {
  ...
  server.tool(
    'list_process_ids',
    'Batch list process instance IDs by modelCode and time range (max 7 days).',
    {
  ...
  server.tool(
    ...
    'Query process instance list with filters (pagination, status, time range).',
    {
  ...
  server.tool(
    'terminate_process',
    'Terminate a running process instance. Only initiator or process admin can operate.',
    {
  ...
  server.tool(
    'recall_process',
    'Recall a process instance. Only initiator or process admin can operate.',
    {
      business_key: z.string().describe('Business key'),
  ...
  server.tool(
    'get_task_history',
    'Get approval history records for a process instance.',
    {
      business_key: z.string().describe('Business key'),
  ...
  server.tool(
    'get_current_tasks',
    'Get current pending tasks and assignees for a process instance.',
    { business_key: z.string().describe('Business key') },
    async ({ business_key }) => {
      try {
  ...
  server.tool(
    'cc_task',
    'Send a task as CC (carbon copy) to users.',
    {
      task_id: z.string().describe('Task ID'),
  ...
  server.tool(
    'sign_task',
    'Add co-signers to a task (before or after current approver).',
    {
      task_id: z.string().describe('Task ID'),
  ...
  server.tool(
    'submit_task',
    'Resubmit a task (after return/sign back).',
    {
      task_id: z.string().describe('Task ID'),
      operator: z.string().describe('Operator user ID'),
      comment: z.string().optional().describe('Comment'),
  ...
  server.tool(
    'get_return_activities',
    'Get the list of activities a task can be returned to.',
    { task_id: z.string().describe('Task ID') },
    async ({ task_id }) => {
      try {
  ...
  server.tool(
    'return_task',
    'Return a task to a previous activity node.',
    {
      task_id: z.string().describe('Task ID'),
  ...
  server.tool(
    ...
    'Get the approval task detail page URL.',
    {
      business_key: z.string().describe('Business key'),
  ...
  server.tool(
    'receive_task',
    'Trigger a receive task to continue process flow.',
    {
      business_key: z.string().describe('Business key'),
  ...
  server.tool(
    'claim_task',
    'Claim a task (for competitive signing scenarios).',
    {
      task_id: z.string().describe('Task ID'),
      operator: z.string().describe('Operator user ID'),
    },
  ...
  server.tool(
    'get_form_definition',
    'Get form definition (UI schema) for a process.',
    {
  ...
  server.tool(
    'get_form_instance',
    'Get form instance data for a process instance.',
    {
