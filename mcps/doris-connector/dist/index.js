...
// Config
// ---------------------------------------------------------------------------
function loadConfig(configPath = 'doris-config.json') {
...
async function main() {
    ...
    if (connNames.length > 1) {
        server.tool('switch_connection', `Switch active Doris connection. Available: ${connList}`, { connection: z.string().describe(`Connection name: ${connList}`) }, async ({ connection }) => {
    ...
    // Query Tools
    // =========================================================================
    server.tool('execute_query', 'Execute a read-only SQL query on Doris (SELECT/SHOW/DESCRIBE).', {
        ...
        try {
            ...
            if (!pool)
                return fail(`Unknown connection "${name}"`);
    ...
    server.tool('list_databases', 'List databases on the Doris cluster.', { connection: z.string().optional().describe('Connection name') }, async ({ connection }) => {
        try {
            const name = connection || currentConnName;
            const pool = pools.get(name);
            if (!pool)
    ...
    server.tool('list_tables', 'List tables in a database.', {
    ...
    server.tool('describe_table', 'Show column definitions for a table.', {
