...
import { ErrorType, } from './kyuubi-types.js';
// ---------------------------------------------------------------------------
...
const PREFIX_TO_REGION = new Map();
 ...
 */
function detectRegion(catalog) {
    for (const [prefix, region] of PREFIX_TO_REGION) {
        if (catalog.includes(prefix))
            return region;
    }
    return undefined;
}
 ...
 */
async function resolveToken(openapi, token) {
    ...
    if (desc.includes('新加坡') || desc.includes('singapore'))
        regionName = 'singapore';
    else if (desc.includes('俄罗斯') || desc.includes('russia') || desc.includes('moscow'))
        regionName = 'russia';
    else if (desc.includes('荷兰') || desc.includes('netherlands') || desc.includes('amsterdam'))
        regionName = 'netherlands';
    else if (desc.includes('印度') || desc.includes('india') || desc.includes('pune'))
        regionName = 'india';
    else if (desc.includes('北京') || desc.includes('beijing') || desc.includes('zjy'))
        regionName = 'beijing';
        ...
        try {
            ...
            if (Array.isArray(catalogs)) {
                for (const cat of catalogs) {
                    ...
                    const catName = typeof cat === 'string' ? cat : (cat.catalogName || cat.name || '');
...
async function loadAndResolveConfig(configPath) {
    ...
    const regionWorkspaces = new Map();
...
// ---------------------------------------------------------------------------
function getClientForCatalog(config, catalog) {
...
function getCurrentWorkspace(config) {
    const ws = config.workspaces.get(config.currentWorkspace);
    if (!ws)
        throw new Error('No active workspace');
    return ws;
}
function getOpenApiClient(config, token) {
 ...
 */
function getClientForWorkspace(config, workspaceId) {
...
async function main() {
    ...
    // Region management
    // =========================================================================
    server.tool('list_regions', '列出所有已配置的区域及工作空间（含 ID、描述，方便区分同区域多空间）', {}, async () => {
        const regions = {};
    ...
    server.tool('switch_region', '切换当前活跃工作空间。可按 region + token_index，或直接按 workspace_id 切换', {
    ...
    // =========================================================================
    server.tool('list_tables', '列出指定 catalog.schema 下的表', {
    ...
    server.tool('describe_table', '查看表的列名和类型。支持全名 catalog.schema.table', {
        table: z.string().describe('Full table name: catalog.schema.table'),
    }, async ({ table }) => {
        try {
            const { catalog, schema, table: tableName } = parseTableName(table);
            const { client } = getClientForCatalog(config, catalog);
            const result = await client.executeQuery(`DESCRIBE ${catalog}.${schema}.${tableName}`, catalog, schema);
    ...
    server.tool('execute_query', '执行只读 SQL（SELECT/SHOW），同步等待结果返回，10 分钟超时', {
    ...
    // =========================================================================
    server.tool('execute_ddl', '执行 DDL 语句（CREATE TABLE / ALTER TABLE / DROP TABLE / CREATE TABLE AS SELECT）。CTAS 自动走异步提交', {
    ...
    // =========================================================================
    server.tool('submit_query', '异步提交 SQL 查询，立刻返回 queryId。适合长查询', {
    ...
    server.tool('check_query', '查看异步查询的状态和执行进度', {
    ...
    server.tool('fetch_query_result', '获取已完成的异步查询结果', {
    ...
    // =========================================================================
    server.tool('get_table_detail', '获取表详情（owner、描述、字段、分区等）。支持全名 catalog.schema.table', {
        table: z.string().describe('Full table name: catalog.schema.table'),
    }, async ({ table }) => {
        try {
            const { catalog, schema, table: tableName } = parseTableName(table);
            ...
            if (regionName) {
                const wsNames = config.regionWorkspaces.get(regionName);
                if (wsNames?.length)
                    token = config.workspaces.get(wsNames[0]).token;
            }
    ...
    server.tool('get_table_fields', '获取表字段定义（名称、类型、注释）', {
        table: z.string().describe('Full table name: catalog.schema.table'),
    }, async ({ table }) => {
        try {
            const { catalog, schema, table: tableName } = parseTableName(table);
    ...
    server.tool('get_table_ddl', '获取建表 DDL 语句', {
        table: z.string().describe('Full table name: catalog.schema.table'),
    }, async ({ table }) => {
        try {
            const { catalog, schema, table: tableName } = parseTableName(table);
    ...
    server.tool('get_table_partitions', '获取分区信息（记录数、文件大小）', {
        table: z.string().describe('Full table name: catalog.schema.table'),
    }, async ({ table }) => {
        try {
            const { catalog, schema, table: tableName } = parseTableName(table);
    ...
    server.tool('search_tables', '按关键词、catalog、库名搜索表', {
    ...
    server.tool('list_databases', '列出 catalog 下的数据库', {
    ...
    server.tool('list_catalogs', '列出可用的 catalog', {
    ...
    // Permission
    // =========================================================================
    server.tool('check_table_permission', '检查当前用户是否有指定表的查询权限', {
        table: z.string().describe('Full table name: catalog.schema.table'),
    }, async ({ table }) => {
        try {
            const { catalog, schema, table: tableName } = parseTableName(table);
    ...
    // Table Management
    // =========================================================================
    server.tool('create_iceberg_table', '通过 OpenAPI 创建 Iceberg 表（支持审批流）。会自动校验字段名、类型、分区字段，自动填充 TBLPROPERTIES 默认值', {
        ...
        // Table name
        if (!VALID_NAME.test(table_name))
            errors.push(`表名 "${table_name}" 不合法，只允许小写字母、数字、下划线，不能以数字开头`);
        if (!catalog.startsWith('iceberg_'))
            errors.push(`Catalog "${catalog}" 必须以 iceberg_ 开头`);
        if (!VALID_NAME.test(database))
            errors.push(`库名 "${database}" 不合法`);
        // Columns
        const colNames = new Set();
        for (const col of columns) {
            if (!VALID_NAME.test(col.name))
                errors.push(`字段名 "${col.name}" 不合法，只允许小写字母、数字、下划线`);
            if (SQL_RESERVED.has(col.name.toLowerCase()))
                errors.push(`字段名 "${col.name}" 是 SQL 保留字，请换一个`);
            if (colNames.has(col.name))
                errors.push(`字段名 "${col.name}" 重复`);
            colNames.add(col.name);
            const upperType = col.type.toUpperCase();
            const isComplex = /^(STRUCT|ARRAY|MAP)</.test(upperType);
            if (!BASE_TYPES.has(upperType) && !isComplex)
                errors.push(`字段 "${col.name}" 的类型 "${col.type}" 不支持。支持：STRING/INT/BIGINT/DOUBLE/FLOAT/BOOLEAN/BINARY/DATE/TIMESTAMP/DECIMAL/STRUCT<>/ARRAY<>/MAP<>`);
        }
        // Partition columns
        if (partition_columns) {
            for (const pc of partition_columns) {
                if (!colNames.has(pc))
                    errors.push(`分区字段 "${pc}" 不在字段列表中`);
                const col = columns.find(c => c.name === pc);
                if (col) {
                    const upperType = col.type.toUpperCase();
                    if (!PARTITION_TYPES.has(upperType))
                        errors.push(`分区字段 "${pc}" 的类型 "${col.type}" 不支持做分区，只允许 INT/BIGINT/STRING/DATE`);
                }
            }
            if (partition_columns.length > 3)
                errors.push(`分区字段 ${partition_columns.length} 个，超过 3 个可能导致小文件问题，建议减少`);
        }
        if (errors.length > 0) {
        ...
        try {
            ...
            const client = getOpenApiClient(config, token);
            const dbs = await client.listDatabases(catalog);
            const dbExists = Array.isArray(dbs) && dbs.some((d) => (d.dbName || d.databaseName || d.name || d) === database);
            if (!dbExists) {
    ...
    // Lineage
    // =========================================================================
    server.tool('query_table_lineage', '查询表的上下游作业血缘', {
        ...
        try {
            ...
            const client = getOpenApiClient(config, token);
            const body = { catalogTableNames: tables };
            if (directions)
    ...
    // =========================================================================
    server.tool('create_sparksql_job', `创建 SparkSQL 作业并自动上线调度+执行。自动填充 noticeList、资源配置等默认值`, {
        ...
        try {
            ...
            let dirMessage = '';
            try {
                const dirTree = await client.viewDirs();
                function findDir(node) {
                    if (!node)
                        return null;
                    if (node.name === defaultUser)
                        return node.id;
                ...
                if (!targetDirId) {
                    const newDir = await client.createDir(defaultUser);
            ...
            const statusParts = [];
            if (!scheduleError)
                statusParts.push('上线调度成功');
            else
                statusParts.push(`上线调度失败: ${scheduleError}`);
            if (taskId)
                statusParts.push(`已触发执行（taskId=${taskId}）`);
            else if (startError)
                statusParts.push(`触发执行失败: ${startError}`);
            // 权限不足时给出友好提示
            let permissionHint = '';
    ...
    server.tool('create_data_push_job', `创建数据推送作业并自动发布。默认推送给自己的飞书，自动填充 noticeList 等配置`, {
        workspace_id: z.number().describe('在哪个工作空间创建'),
        ...
        try {
            ...
            try {
                ...
                if (!targetDirId) {
                    ...
                    const newDir = await client.createDir(targetDirName);
                    targetDirId = typeof newDir === 'number' ? newDir : (newDir?.id ?? null);
                }
                if (targetDirId) {
                    await client.moveJobsToDir(targetDirId, [String(jobId)]);
    ...
    // Notebook
    // =========================================================================
    server.tool('create_notebook', '在工作空间中创建一个 Notebook 文件', {
    ...
    server.tool('create_notebook_job', '创建 Notebook 调度作业并自动上线+执行', {
        workspace_id: z.number().describe('工作空间 ID'),
        ...
        try {
            ...
            if (output_notebook_path)
                notebook.outputNotebookPath = output_notebook_path;
            if (git_url)
                notebook.gitUrl = git_url;
            if (git_ref)
                notebook.gitRef = git_ref;
            if (parameters)
    ...
    server.tool('update_notebook_job', '修改 Notebook 作业', {
        ...
        try {
            ...
            if (notebook_path || output_notebook_path || git_url || git_ref) {
                const notebook = {};
                if (notebook_path)
                    notebook.inputNotebookPath = notebook_path;
                if (output_notebook_path)
                    notebook.outputNotebookPath = output_notebook_path;
                if (git_url)
                    notebook.gitUrl = git_url;
    ...
    // =========================================================================
    server.tool('update_sparksql_job', '修改 SparkSQL 作业', {
        workspace_id: z.number().optional().describe('工作空间 ID'),
        job_id: z.string().describe('作业 ID'),
    ...
    server.tool('update_data_push_job', '修改数据推送作业', {
        workspace_id: z.number().optional().describe('工作空间 ID'),
        job_id: z.string().describe('作业 ID'),
        body: z.record(z.unknown()).describe('更新的作业配置'),
    }, async ({ workspace_id, job_id, body }) => {
    ...
    server.tool('get_job_detail', '获取作业详情', {
        workspace_id: z.number().optional().describe('工作空间 ID'),
        job_id: z.string().describe('作业 ID'),
    ...
    // =========================================================================
    server.tool('enable_job_schedule', '启用作业调度', {
        workspace_id: z.number().optional().describe('工作空间 ID'),
        job_id: z.string().describe('作业 ID'),
    ...
    server.tool('disable_job_schedule', '停用作业调度', {
        workspace_id: z.number().optional().describe('工作空间 ID'),
        job_id: z.string().describe('作业 ID'),
    }, async ({ workspace_id, job_id }) => {
        try {
     ...
     */
    async function resolveTaskId(client, dagNodeId) {
        const result = await client.listDagNodeTasks(dagNodeId, 1, 10);
    ...
    server.tool('get_task_detail', '获取任务实例详情。如果不指定 workspace_id，会自动遍历所有工作空间查找', {
         ...
         */
        async function tryGetDetail(client, id, wsInfo) {
            try {
                const result = await client.getTaskDetail(id);
                return ok(wsInfo ? { workspace: wsInfo, ...result } : result);
            }
            catch (err) {
                ...
                if (/kolibre-adapter|B-0|HTTP 500/i.test(msg)) {
                    ...
                    return ok(wsInfo
                        ? { workspace: wsInfo, dagNodeId: id, resolvedTaskId: taskId, ...detail }
                        : { dagNodeId: id, resolvedTaskId: taskId, ...detail });
                }
                throw err;
            }
        }
    ...
    server.tool('get_task_log', '获取任务实例日志。如果不指定 workspace_id，会自动遍历所有工作空间查找', {
        workspace_id: z.number().optional().describe('工作空间 ID（不传则自动遍历所有空间查找）'),
        task_id: z.string().describe('任务实例 ID'),
    }, async ({ workspace_id, task_id }) => {
        async function tryGetLog(client, id, wsInfo) {
            try {
                ...
                if (/kolibre-adapter|B-0|HTTP 500/i.test(msg)) {
                    ...
                    return ok(wsInfo
                        ? { workspace: wsInfo, dagNodeId: id, resolvedTaskId: taskId, ...log }
                        : { dagNodeId: id, resolvedTaskId: taskId, ...log });
                }
                throw err;
            }
        }
