...
export class OpenApiClient {
  ...
  async listAuthorizedTables(opts: {
    keyword?: string; catalog?: string; dbName?: string;
    service?: string; isGranted?: boolean; isIncludePartitionTable?: boolean;
  }): Promise<unknown> {
    return this.get('/metadata/table/authorized/list', opts as Record<string, unknown>);
  }

  /** GET /metadata/table/get — 获取表详情 */
  async getTableDetail(catalog: string, dbName: string, tableName: string): Promise<unknown> {
    return this.get('/metadata/table/get', { catalog, dbName, tableNameEn: tableName });
  }

  /** GET /metadata/table/field/list — 获取表字段 */
  async getTableFields(catalog: string, dbName: string, tableName: string): Promise<unknown> {
    return this.get('/metadata/table/field/list', { catalog, dbName, tableName });
  }

  /** GET /metadata/table/table/show/ddl — 获取建表 DDL */
  async getTableDDL(catalog: string, dbName: string, tableName: string): Promise<unknown> {
    return this.get('/metadata/table/table/show/ddl', { catalog, dbName, tableName });
  }

  /** GET /metadata/table/partition/list — 获取表分区 */
  async getTablePartitions(catalog: string, dbName: string, tableName: string): Promise<unknown> {
    return this.get('/metadata/table/partition/list', { catalog, dbName, tableName });
  }

  /** GET /metadata/table/log/get — 获取表变更日志 */
  async getTableLog(catalog: string, dbName: string, tableName: string): Promise<unknown> {
  ...
  async listCatalogs(engine?: string, service?: string): Promise<unknown> {
    return this.get('/metadata/catalog/list', { engine, service });
  }

  /** GET /database/list — 库列表 */
  async listDatabases(catalog: string, keyword?: string): Promise<unknown> {
  ...
  async queryTableLineage(body: Record<string, unknown>): Promise<unknown> {
  ...
  async createSparkSQLJob(body: Record<string, unknown>): Promise<unknown> {
  ...
  async updateSparkSQLJob(jobId: string, body: Record<string, unknown>): Promise<unknown> {
    return this.put(`/develop/jobs/${jobId}/op/sparksql`, body);
  }

  /** POST /develop/jobs/op/data/push — 新建数据推送作业 */
  async createDataPushJob(body: Record<string, unknown>): Promise<unknown> {
    return this.post('/develop/jobs/op/data/push', body);
  }

  /** POST /develop/jobs/op/notebook — 新建 Notebook 作业 */
  async createNotebookJob(body: Record<string, unknown>): Promise<unknown> {
    return this.post('/develop/jobs/op/notebook', body);
  }

  /** PUT /develop/jobs/{jobId}/op/notebook — 修改 Notebook 作业 */
  async updateNotebookJob(jobId: string, body: Record<string, unknown>): Promise<unknown> {
    return this.put(`/develop/jobs/${jobId}/op/notebook`, body);
  }

  /** POST /develop/notebook/create — 创建 Notebook 文件 */
  async createNotebook(body: Record<string, unknown>, user: string, workspaceId: number): Promise<unknown> {
  ...
  async checkTablePermission(catalog: string, dbName: string, tableName: string, userName: string, workspaceId: number): Promise<unknown> {
  ...
  async startJob(jobId: string, version?: number): Promise<unknown> {
    const params = version !== undefined ? `?version=${version}` : '';
    return this.postRaw(`/develop/jobs/${jobId}/start${params}`);
  }

  /** POST /develop/jobs/{jobId}/stop — 停止作业 */
  async stopJob(jobId: string, taskId?: string): Promise<unknown> {
  ...
  async listJobs(opts: {
    searchKey?: string; jobId?: string; jobTypes?: string;
    lastScheduleStatus?: string; owner?: boolean;
    page?: number; pageSize?: number;
  }): Promise<unknown> {
    return this.get('/develop/jobs/op/list', opts as Record<string, unknown>);
  }
  ...
  async enableJobSchedule(jobId: string): Promise<unknown> {
    return this.postRaw(`/develop/jobs/${jobId}/op/scheduler/start`);
  }

  /** POST /develop/jobs/{jobId}/op/scheduler/stop — 停用调度 */
  async disableJobSchedule(jobId: string): Promise<unknown> {
  ...
  async listJobDagNodes(jobId: string, page?: number, pageSize?: number): Promise<unknown> {
    return this.get(`/develop/jobs/${jobId}/dagNodes`, { page: page ?? 1, pageSize: pageSize ?? 20 });
  }

  /** GET /develop/dagNodes/{dagNodeId} — dagNode 详情 */
  async getDagNodeDetail(dagNodeId: string): Promise<unknown> {
    return this.get(`/develop/dagNodes/${dagNodeId}`);
  }

  /** GET /develop/dagNodes/{dagNodeId}/tasks — dagNode 下的 task 实例列表 */
  async listDagNodeTasks(dagNodeId: string, page?: number, pageSize?: number): Promise<unknown> {
  ...
  async getTaskLog(taskId: string): Promise<unknown> {
  ...
  async viewDirs(): Promise<unknown> {
    return this.get('/develop/dir/view');
  }

  /** POST /develop/dir/new — 新建分组 */
  async createDir(name: string, parentDirId?: number): Promise<unknown> {
    const qs = new URLSearchParams();
    qs.set('name', name);
    if (parentDirId !== undefined) qs.set('parentDirId', String(parentDirId));
    return this.postRaw(`/develop/dir/new?${qs.toString()}`);
  }

  /** POST /develop/dir/move/batch/job — 批量移动作业到目录 */
  async moveJobsToDir(dirId: number, jobIds: string[]): Promise<unknown> {
    const qs = new URLSearchParams();
  ...
  async getWorkspaceInfo(): Promise<{
    id: number; workspaceName: string; description: string;
    owner: string; role: string; department: string;
  }> {
    return this.get('/workspace/info') as any;
  }

  /** GET /workspace/token/detail — 获取当前 Token 信息（含 token 对应的用户名） */
  async getTokenDetail(): Promise<{ user: string; role: string; workspaceId: number }> {
    return this.get('/workspace/token/detail') as any;
  }

  /** GET /common/workspace/detail — 获取空间基本信息（含 regionId/regionName） */
  async getWorkspaceDetail(): Promise<{ id: number; workspaceName: string; regionId: string; regionName: string; owner: string; description: string }> {
    return this.get('/common/workspace/detail') as any;
  }

  /** GET /common/region/list — 区域列表 */
  async listRegions(): Promise<unknown> {
  ...
  private async postRaw(path: string): Promise<unknown> {
    const url = `${this.baseUrl}${path}`;
    const resp = await fetch(url, {
      method: 'POST',
      headers: this.headers(),
    ...
    if (json && typeof json === 'object' && !Array.isArray(json) && ('status' in json || 'code' in json) && 'msg' in json) {
      ...
      if (hasError) {
        throw new KyuubiError(
          ...
          `API Error: ${wrapped.msg || 'unknown'} (status: ${wrapped.status}, code: ${wrapped.code})`,
